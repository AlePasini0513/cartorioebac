...
int registro() //função responsavel por cadstrar os usuarios no sistema
{
	char arquivo[40]; //inicio criação variaveis/srings
	char cpf[40];
	char nome[40];
	char sobrenome[40];
	char cargo[40]; //final da criação de variaveis/strings
	
	printf("DIgite o CPF a ser cadastrado: "); //coletando informção de usuario
...
int main()
{
	...
	if(comparacao == 0){
		...
		for(x=1;x=1;)
		{
			...
			switch(opcao) //incio de seleção do menu
			{
				case 1:
